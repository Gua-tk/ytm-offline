...
class Global:
    ...
    def download_playlist(self):
        ...
        compressionService.zip_folder(os.path.join(current_download_path, directory_name), os.path.join(current_download_path, directory_name + ".zip"))
    ...
    #@app.route('/uploadPlaylist', methods=['POST'])
    ...
    #@app.route('/uploadToPlaylist', methods=['POST'])
    def upload_to_playlist(self):
        data = request.json
        playlist_url = data['playlist_url']
        request_uuid = uuid.uuid4()
        current_download_path = os.path.join(download_path, "playlists", str(request_uuid))
